- AdvantageKit/EnergyLogger 日志常见两类可恢复不完整性：文件尾部最后一条 record 被截断（包括部分 record 后残留对齐的 4 KiB 全零未写入块），以及 V2.3 电机断连时整机 Supply Current 短暂写为 `NaN`。全零尾块必须从被掩盖的 record 起点回退，不能把零值补齐的伪 record 计入；零块后存在非零字节仍是中段损坏。解析器只能把“中段结构损坏”或“全程没有任何有限电气区间”视为 fatal；局部 `NaN` 区间必须保留为空洞、停止该区间积分并继续恢复后的数据，同时让完全落在空洞内的峰值显示为无可用数据，不能伪装成 0。
